# CODE-BINGO
Real-time multiplayer 5x5 bingo web app — first to complete 5 lines wins, last caller wins ties. Rooms, random/manual boards, WebRTC voice chat.

# Bingo 5x5 - First to 5 Lines Wins

A real-time multiplayer 5x5 bingo web app. Players arrange (or randomly shuffle) a board of numbers 1-25, numbers are drawn one at a time, and the first player to complete **5 lines** (rows, columns, or diagonals) hits BINGO. If multiple players finish on the same call, **the last caller wins**.
...
- **Real-time play** - host draws balls (or enables auto-call), players tap to mark
...
- **Server-side rules** - marking, line counting, and claims are validated on the server
...
3. Host clicks **Start game** and draws balls (or enables **Auto-call** with a chosen interval).
4. When your number is called, tap its cell to mark it.
...
```
server.js          Node/Express/WebSocket server - rooms, draw, marks, claims
...
- Numbers 1-25 are drawn without replacement.
